...
//
// IMPORTANTE: o arquivo de Troca de Cedente e IMPORTADO diretamente
// no sistema do BMP (endpoint ib/v1/CNAB400/ExtrairInformacoesArquivo),
// que le o layout PADRAO do CNAB400 (o mesmo dos arquivos de Registro
// que o BMP ja aceita: registros 0/1/2/9, juros/mora em 161-173, SEM
// registro tipo 4). Por isso reaproveitamos os builders padrao
// (buildHeader/buildDetalhe1/buildDetalhe2/buildTrailer) e NAO o
// layout do PDF (0/1/2/4). O que muda em relacao a um Registro comum:
//  - contaRecebedor = dados da CAPT CAPITAL -> vao no HEADER (cedente,
//    nome e conta), independentemente do perfil logado.
//  - contaOriginal  = cedente ORIGINAL dos boletos (perfil logado) ->
//    vai no DETALHE (carteira/agencia/conta em 021-037/030-037), que e
//    o que identifica o titulo existente no banco para a troca.
// Se o BMP exigir que o detalhe tambem use a conta da CAPT, basta
// passar contaRecebedor no lugar de contaOriginal no buildDetalhe1.
// ============================================================
export const generateCNAB400TrocaCedenteFile = async (boletos, contaRecebedor, contaOriginal, nextSeq, onProgress) => {
    if (!boletos || boletos.length === 0) {
    ...
    const seq = nextSeq || (contaRecebedor.cnab400 ? Number(contaRecebedor.cnab400) + 1 : 1)
    const lines = []
    let lineSeq = 1

    // Header = layout padrao, mas com os dados da CAPT CAPITAL (recebedor)
    lines.push(buildHeader(contaRecebedor, seq, '01'))
    lineSeq++
    ...
    let idx = 0
    for (const boleto of boletos) {
        // Detalhe = layout padrao, identificando o cedente ORIGINAL (perfil logado)
        lines.push(buildDetalhe1(boleto, contaOriginal, lineSeq, '01')); lineSeq++
        lines.push(buildDetalhe2(boleto, lineSeq));                      lineSeq++
        idx++
        if (idx % loteYield === 0) {
    ...
    lines.push(buildTrailer(lineSeq))

    // Padrao BMP: substitui pontos por espacos no conteudo final
    const content = lines.join('\r\n').replace(/\./g, ' ')
    console.log('[CNAB400-Troca] Remessa (layout padrao) gerada:', lines.length, 'linhas, seq:', seq)
