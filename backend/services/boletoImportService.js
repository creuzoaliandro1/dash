...
async function processarBoletoComCache(
  ...
  try {
    ...
    // Fazer partial match como na versão original (ilike com %)
    let contaEncontrada = null;
    for (const [chave, conta] of contasCache.entries()) {
      if (chave.startsWith(String(numeroConta).trim())) {
...
export async function processarArquivoBoletos(boletos, usuarioLogado, supabase, perfil = 'normal') {
  ...
  todasContas.forEach(c => {
    // Buscar contas com ILIKE (case-insensitive partial match) como na versão original
    // Armazenar a conta como string trimmed
    const contaStr = String(c.conta || '').trim();
    if (contaStr) {
      contasCache.set(contaStr, c);
    }
  });
  console.log(`✅ Cache: ${contasCache.size} contas carregadas`);
  ...
  const durracaoCache = ((Date.now() - startCache) / 1000).toFixed(2);
  console.log(`⏱️  Tempo de cache: ${durracaoCache}s`);

  // ===== OTIMIZAÇÃO: Processar em lotes paralelos (100 por vez) =====
  const startProcessamento = Date.now();
  const LOTE_SIZE = 100;
  const logsParaBatch = [];
  ...
  const durracaoProcessamento = ((Date.now() - startProcessamento) / 1000).toFixed(2);
  console.log(`⏱️  Tempo de processamento: ${durracaoProcessamento}s`);
  ...
  if (boletosParaInserir.length > 0) {
    ...
    if (erroInsert) {
      ...
      const durracaoInsert = ((Date.now() - startInsert) / 1000).toFixed(2);
      console.log(`✅ ${dadosInseridos.length} boletos inseridos em ${durracaoInsert}s`);
  ...
  if (boletosParaAtualizar.length > 0) {
    ...
    const durracaoUpdate = ((Date.now() - startUpdate) / 1000).toFixed(2);
    console.log(`✅ ${boletosParaAtualizar.length} boletos atualizados em ${durracaoUpdate}s`);
  }
